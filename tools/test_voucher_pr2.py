...
# 投影后的 SQL（与 patches_pr2.md 中 Patch 3-* 一致）
PROJECTED_SQL = {
    "SS": (
...
def test_d1_not_present():
    """PR-2 干净版：完全不包含 D1 投影列方案"""
    with open("src/voucher/patches_pr2.md") as f:
        content = f.read()
    # 干净版根本不应该提投影列
    bad_signs = ["主查询投影列", "SELECT M.BillID, M.BillNo"]
    for s in bad_signs:
        if s in content:
            print(f"[FAIL] PR-2 干净版仍含 D1 残留: {s!r}")
            return 1
    print(f"[OK ] d1_not_present: PR-2 干净版不含 D1 投影列内容")
    return 0
...
def main():
    ...
    print("\n--- test_d1_not_present（确保干净版不含 D1）---")
    f4 = test_d1_not_present()
